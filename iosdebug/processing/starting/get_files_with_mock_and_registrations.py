...
def get_files_with_mock_and_registrations(repository_protocols, path_to_content_map, protocol_to_protocol_functions):
    files_with_mocks = []
    registrations = []
        ...
        for path in path_to_content_map:
            if 'class ' + protocol in path_to_content_map[path]:
                ...
                for function in protocol_to_protocol_functions[protocol]:
                    ...
                    processed_func_template = processed_func_template.replace('<FUNC_NAME>', function.name)
                    if function.params:
                        processed_func_template = processed_func_template.replace('<FUNC_PARAMS>',
                                                                                  ', '.join(function.params))
                    else:
                        processed_func_template = processed_func_template.replace('<FUNC_PARAMS>', '')
                    processed_func_template = processed_func_template.replace('<CASES>', '\n        '.join(
                    ...
                    if function.params:
                        ...
                        func_call = '()'
                    processed_func_template = processed_func_template.replace('<ORIGINAL_FUNC_CALL>',
                ...
                processed_template = processed_template.replace('<FUNCTIONS>', '\n    '.join(functions))

                with open(path, 'a') as file:
                    file.write(processed_template)
                files_with_mocks.append(path)
                registrations.append("registerMock<PROTOCOL>(to: container)".replace('<PROTOCOL>', protocol))
