...
def create_and_set_mock_manager(swift_files, path_to_content_map, processed_mock_manager):
    for file_path in swift_files:
        with open(file_path, 'r') as file:
            ...
            if root_vc_property:
                ...
                if '/ MARK: - Debug mode helper classes' not in content or True:
                    with open(DATA_FILE, 'rb') as file:
                        data = pickle.load(file)
                        data = literal_eval(data)
                        data['original_root_view_controller'] = root_vc_property_definition
                        data['changed_root_view_controller'] = changed
