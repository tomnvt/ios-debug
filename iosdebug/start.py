...
def start():
    ...
    protocol_to_protocol_functions = get_protocols_to_protocol_functions_dict(repository_protocols, path_to_content_map)
    files_with_mocks, registrations = get_files_with_mock_and_registrations(
        repository_protocols, path_to_content_map, protocol_to_protocol_functions
    )
    processed_mock_manager = get_mock_settings_and_mock_cursor(protocol_to_protocol_functions)
    protocol_to_mocked_contents = get_protocol_to_mocked_content_dict(repository_protocols, path_to_content_map)
    store_mocked_implementation(protocol_to_mocked_contents)
    create_and_set_mock_manager(swift_files, path_to_content_map, processed_mock_manager)
